function [ LogDomain , ProbabilityDomain , s_map ] = BCJR_NC( TRANS , rx_noisless , SNRdB , PR_Trellis , tx_waveform , Trellis_Index_io)
...
State_Cardinality=size(PR_Trellis,1);
 ...
 rx_waveform = tx_waveform...
                 +normrnd(0,sigma/sqrt(2),1,length(tx_waveform))...
                 +1i*normrnd(0,sigma/sqrt(2),1,length(tx_waveform));
    ...
    %% gamma Calculation
                    for X=1:size(rx_noisless,1)
                        if [permute(PR_Trellis(state_i,state_j,branchIndex,:),[4 1 2 3])]'==rx_noisless(X,:)
                              ar=[prod(...
                                  (1./(2*pi*(sigma^2))).*exp( (-1/(2*(sigma^2)))*( (abs(rx_waveform_pac))^2 + (abs(rx_noisless(X,:)))^2 ) )...
                                  .*(...
                                  besseli((sigma^-2)*abs(sum()),1)...
                                  ./...
                                  besseli((sigma^-2),1)...
                                   )...
                                  ,2)]';
